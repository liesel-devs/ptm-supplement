...
import jax
from liesel_ptm import term, term_ri, lin, ri

jax.config.update("jax_enable_x64", True)


parser = argparse.ArgumentParser()
parser.add_argument("--jobdir", type=str, default="application-fh/002-ptm-linear")
...
mod = ptm.LocScalePTM.new_ptm(
    response=train["cholst"].to_numpy(),
    a=params["a"],
    b=params["b"],
    nparam=params["nparam"],
    trafo_target_slope=params["trafo_target_slope"],
    to_float32=False,
)

lin_age = lin(train["age"], xname="age")  # Linear basis
mod.loc += term.f(lin_age, fname="s")
mod.scale += term.f(lin_age, fname="g")
...
mod.loc += term_ri.f_ig(ri_id, ig_concentration=1.0, ig_scale=0.001, fname="ri")
...
# get basis matrix in the right shape
newdata["id"] = jnp.full_like(newdata["age"], fill_value=-1)
...
# ..............................................................................
# ---- Z values for diagnosis ----
# ..............................................................................
df = data
newdata = {"age": df["age"].to_numpy()}
newdata["sex"] = df["sex"].to_numpy()
newdata["id"] = df["newid"].to_numpy()

dist = mod.init_dist(samples, newdata=newdata)

z_samples = dist.transformation_and_logdet(df["cholst"].to_numpy())[0]

z_summary = (
    gs.SamplesSummary.from_array(z_samples, name="z").to_dataframe().reset_index()
)

z_summary["in_training_data"] = data["newid"].isin(train["newid"])
z_summary["in_training_data"] = pd.Categorical(z_summary["in_training_data"])


# ..............................................................................
# ---- Standardized conditional distribution ----
# ..............................................................................

dist = mod.init_dist(samples, loc=0.0, scale=1.0)

r_grid = jnp.linspace(-5.0, 7.0, 250)  # type: ignore

log_prob_samples = dist.log_prob(r_grid)

prob_summary = (
    gs.SamplesSummary.from_array(jnp.exp(log_prob_samples), name="pdf")
    .to_dataframe()
    .reset_index()
)
prob_summary["r"] = r_grid
prob_summary["pdf_gaussian"] = tfd.Normal(loc=0.0, scale=1.0).prob(r_grid)
...
for name, df in summaries.items():
    ...
    df["a"] = params["a"]
    df["b"] = params["b"]
    df["nparam"] = params["nparam"]
    df["trafo_target_slope"] = params["trafo_target_slope"]
