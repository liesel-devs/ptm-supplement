...
import pandas as pd
from liesel_ptm import summarise_by_samples
...
def covariate_df(f: str, xnum, scale: bool, test, newdata, samples):
    suffix = ""
    match f:
        case "s":
            suffix = "loc"
    ...
    pred_summary = pd.DataFrame({"fx_hat": pred.mean(axis=(0, 1))})
    pred_summary["low"] = jnp.quantile(pred_centered, q=0.05, axis=(0, 1))
    pred_summary["high"] = jnp.quantile(pred_centered, q=0.95, axis=(0, 1))
    pred_summary["fx_true"] = fx_centered.squeeze()
    pred_summary["x"] = test[f"x{xnum}"].to_numpy()
    pred_summary["parameter"] = suffix
    pred_summary["xnum"] = xnum

    nsamples = 50
    pred_samples_summary = summarise_by_samples(
        jax.random.key(1), pred, n=nsamples, name="fx_hat"
    )
    pred_samples_summary["x"] = jnp.tile(
        jnp.squeeze(test[f"x{xnum}"].to_numpy()), nsamples
    )
    pred_samples_summary["fx_true"] = jnp.tile(jnp.squeeze(fx_centered), nsamples)
    pred_samples_summary["parameter"] = suffix
    pred_samples_summary["xnum"] = xnum

    return pred_summary, pred_samples_summary
